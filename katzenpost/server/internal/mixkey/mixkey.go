...
// Init initializes a MixKey from existing public and private keys without
// generating new ones. Used for loading persisted keys. The bloom filter is
// initialized fresh (replay protection restarts on reload, which is acceptable).
func Init(epoch uint64, pubKey nike.PublicKey, privKey nike.PrivateKey, kemPubKey kem.PublicKey, kemPrivKey kem.PrivateKey) (*MixKey, error) {
	k := &MixKey{
		epoch:    epoch,
		refCount: 1,
	}
	if pubKey != nil && privKey != nil {
		k.nikePubKey = pubKey
		k.nikeKeypair = privKey
	} else if kemPubKey != nil && kemPrivKey != nil {
		_ = kemPubKey
		k.kemKeypair = kemPrivKey
	}
	var err error
	k.f, err = bloom.New(rand.Reader, 29, 0.001)
	if err != nil {
		return nil, err
	}
	return k, nil
