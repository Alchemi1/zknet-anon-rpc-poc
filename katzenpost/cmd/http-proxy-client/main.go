...
func (k *kttp) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// normalize the request URL so the proxy server can route the upstream
	// request correctly (the upstream host may only be present in the Host
	// header, and the scheme is absent when a path-only request arrives)
	if r.URL.Host == "" {
		r.URL.Host = r.Host
	}
	if r.URL.Scheme == "" {
